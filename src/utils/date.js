 ...
 * @returns
 */
export function getDATE(date, splitor = '-',years) {
  ...
  if(!years){
    ...
    return `${addZeroPrefix(month)}${splitor}${addZeroPrefix(day)}`
  }
...
// 身份证获取年龄
export function ages(cardNo) {
  var myDate = new Date()
  var month = myDate.getMonth() + 1
  var day = myDate.getDate()
  var age = myDate.getFullYear() - cardNo.substring(6, 10) - 1
  if (cardNo.substring(10, 12) < month || cardNo.substring(10, 12) === month &&
    cardNo.substring(12, 14) <= day) {
    age++
  }
  return age
}
