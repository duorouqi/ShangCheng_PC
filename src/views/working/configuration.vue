...
<template>
	<div id="title" style="height: 40px; width: auto; background-color: #fbfbfb; ">
		...
		<span style="font-size: 14px; font-weight:100;">标准户型缩略图查询</span>
	</div>
	<div class="box">
		<div id="box-conet">
			<div class="tops">
				<el-form style="display: flex; margin-top: 5px;">
					<el-form-item label="定位" style="margin-right: 15px;margin-left: 15px;">
					...
					<el-form-item label="层数" style="margin-right: 15px;">
					...
					<el-form-item label="套型" style="margin-right: 15px;">
					 ...
					 <div v-for="(itmer,index) in publicer.result" :key="index" @click="submit(itmer.mainTable.id)" style="margin: 10px 1.5vw; border-radius: 4px;overflow: hidden;border: 1px solid #ebebeb; width: 30%; height: auto;"><!--margin-left: 10px;margin-bottom: 10px;padding-bottom: 15px; -->
						...
						<div style="margin: 15px 0;">
							...
							<div style="display: flex;">
								...
								<div>
									<el-tooltip class="item" effect="dark" :content="itmer.mainTable?itmer.mainTable.tx:''" placement="right">
									  <p style="margin-left: 15px;white-space:nowrap; overflow:hidden; text-overflow:ellipsis;width: 21vw;font-size: 1vw;">{{itmer.mainTable?itmer.mainTable.tx:''}}</p>    
									</el-tooltip>
								</div>
							</div>
							<div style="display: flex;">
...
<script setup>
	...
	let submit = (id) =>{
		let url = "https://oa.siud.com/spa/cube/index.html#/main/cube/card?type=0&modeId=3921&formId=-689&billid="+id
		window.open(url);
	}
	...
	let gettotal = async () => {
		try {
			...
			let bzktotal = await getbzktotal(JSON.stringify({
				"operationinfo":{
					"operator":formData.value.id
				},
				"mainTable":{
				...
				"header":{
					"systemid":"bzkTotal",
					"currentDateTime":setTime(),
					"Md5":md5("bzkTotal"+"123"+setTime())
				}
			})); 
				count.value = eval("(" +  bzktotal.result + ")").pageCount
			} catch (e) {
				console.log(e)
				//TODO handle the exception
			}
